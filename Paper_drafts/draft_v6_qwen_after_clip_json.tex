...
\begin{enumerate}
    \item \textbf{Exp 1A:} Base CLIP, frozen encoder
    \item \textbf{Exp 1B:} Fine-tuned CLIP, frozen encoder
    \item \textbf{Exp 1C:} Fine-tuned CLIP, unfrozen encoder (in progress)
    \item \textbf{Exp 1D:} Base CLIP, unfrozen encoder
\end{enumerate}

\paragraph{Results.} Table~\ref{tab:json_ablation} shows the ablation study results:

\begin{table}[h]
\centering
\caption{JSON-First Model Ablation Study Results}
\label{tab:json_ablation}
\begin{tabular}{lccc}
\toprule
\textbf{Experiment} & \textbf{Configuration} & \textbf{Per-Square Acc.} & \textbf{Exact Match} \\
\midrule
Exp 1A & Base CLIP, Frozen & 79.31\% & 0.008\% \\
Exp 1B & Fine-tuned CLIP, Frozen & 79.32\% & 0.008\% \\
Exp 1D & Base CLIP, Unfrozen & 79.13\% & 0.02\% \\
\midrule
\textbf{Best} & \textbf{Fine-tuned CLIP, Frozen} & \textbf{79.32\%} & \textbf{0.008\%} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
    \item \textbf{Chess fine-tuning impact:} Minimal benefit (+0.01\%) when encoder is frozen, suggesting pre-trained CLIP features are already sufficient
    \item \textbf{End-to-end training impact:} Unfreezing the encoder actually \emph{hurts} performance (-0.18\%), indicating that freezing preserves pre-trained features better
    \item \textbf{Per-square accuracy:} Achieves 79.32\% accuracy, a significant improvement over 0\% exact match in generative FEN decoding
...
\paragraph{VLM Fine-tuning Comparison.} To explore whether vision-language models can directly learn JSON prediction, we fine-tuned Qwen2-VL-2B-Instruct using LoRA (Low-Rank Adaptation) on the same dataset. The model was trained for 3 epochs (189 steps) with a learning rate of $2 \times 10^{-4}$, achieving a training loss reduction from 43.64 to 16.50. However, evaluation on 50 test samples reveals significant limitations:

\begin{itemize}
    \item \textbf{Valid JSON rate:} 46.00\% (23/50) --- the model learns JSON structure
    \item \textbf{Valid position rate:} 12.00\% (6/50) --- only 12\% represent valid chess positions
    \item \textbf{Per-square accuracy:} 43.55\% (averaged over valid JSON samples)
    \item \textbf{Exact board match:} 0.00\%
\end{itemize}

Comparison with our CLIP-based JSON predictor shows that the grid-based classification approach (79.32\% per-square accuracy) significantly outperforms VLM fine-tuning (43.55\%), with a difference of -35.78\%. This suggests that:

\begin{enumerate}
    \item \textbf{Grid-based classification is better suited} for structured spatial tasks like chess position recognition
    \item \textbf{VLMs struggle with precise spatial reasoning} even when fine-tuned, generating valid JSON syntax but incorrect chess content
    \item \textbf{Parallel classification avoids exposure bias} more effectively than autoregressive text generation
\end{enumerate}
