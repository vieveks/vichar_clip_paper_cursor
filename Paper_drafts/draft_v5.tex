...
To evaluate the impact of providing FEN context to VLMs, we conduct a comprehensive benchmark using GPT-4o on 10 chess positions with 8 different question types (80 total question-answer pairs).
...
\paragraph{Question Types.} We evaluate on 8 question types covering various aspects of chess understanding:
\begin{enumerate}
    ...
    \item \textbf{Material Balance:} Compare material values
...
\paragraph{Results.} Table~\ref{tab:vlm_results} shows the performance improvement when FEN context is provided. Overall, we observe a 68.3\% relative improvement in average score (from 0.126 to 0.212), demonstrating that symbolic grounding significantly enhances VLM performance on chess understanding tasks.
...
Each sample consists of a 512×512 pixel chess board image and its corresponding FEN (Forsyth-Edwards Notation) string. For VLM evaluation, we use a subset of 10 test positions, resulting in 80 question-answer pairs (10 positions × 8 question types).

\subsection{Question Examples}

We evaluate on 8 question types covering various aspects of chess understanding:
...
Table~\ref{tab:detailed_results} provides per-question performance metrics:

\begin{table}[h]
\centering
\caption{Detailed Per-Question Performance Metrics}
\label{tab:detailed_results}
\begin{tabular}{lcccc}
\toprule
\textbf{Question Type} & \textbf{Avg Score (No FEN)} & \textbf{Avg Score (+ FEN)} & \textbf{Accuracy (No FEN)} & \textbf{Accuracy (+ FEN)} \\
\midrule
FEN Extraction   & 0.13 & 0.46 & 0\% & 40\% \\
Check Status     & 0.05 & 0.40 & 0\% & 0\% \\
Material Balance & 0.27 & 0.30 & 0\% & 0\% \\
Piece Count      & 0.30 & 0.30 & 30\% & 30\% \\
Best Move        & 0.26 & 0.24 & 10\% & 0\% \\
Tactical Pattern & 0.00 & 0.00 & 0\% & 0\% \\
Castling Rights  & 0.00 & 0.00 & 0\% & 0\% \\
Piece Location   & 0.00 & 0.00 & 0\% & 0\% \\
\bottomrule
\end{tabular}
\end{table}
...
\paragraph{Image Preprocessing Pipeline.} For enhanced and consensus strategies:
\begin{enumerate}
    \item Load board image (from PDF page or image file)
    \item Apply contrast enhancement: multiply pixel values by 1.3, clip to [0, 255]
    \item Apply sharpness enhancement: use PIL ImageFilter.SHARPEN with 1.5x strength
    \item Apply brightness adjustment: multiply by 1.1, clip to [0, 255]
    \item Upscale if needed: if longest edge < 1024px, upscale using LANCZOS resampling
    \item Convert to base64 data URI for API transmission
\end{enumerate}

\paragraph{Prompt Template.} Enhanced prompts include:
\begin{verbatim}
Analyze this chess board image and provide the FEN notation.

Instructions:
1. Scan the board rank by rank (from rank 8 to rank 1)
2. For each rank, list pieces from left to right (a-file to h-file)
3. Use uppercase for White pieces (K, Q, R, B, N, P)
4. Use lowercase for Black pieces (k, q, r, b, n, p)
5. Use numbers for consecutive empty squares
6. Separate ranks with '/'
7. Verify: both kings present, exactly 8 ranks, proper notation

Provide the FEN string only.
\end{verbatim}

\paragraph{Consensus Algorithm.} For consensus strategy:
\begin{enumerate}
    \item Generate FEN 3--5 times independently (with same preprocessing)
    \item Validate each FEN for syntax correctness
    \item Count occurrences of each unique FEN
    \item Select FEN with highest count (majority vote)
    \item Calculate confidence: (count / total\_attempts) × 100\%
    \item Return FEN with confidence score
\end{enumerate}

\paragraph{Error Handling.} The pipeline includes robust error handling:
\begin{itemize}
    \item API rate limit detection and automatic retry with exponential backoff
    \item Invalid FEN detection and error reporting
    \item Board detection failures (no boards found) are logged but don't stop processing
    \item Partial results saved even if some boards fail
\end{itemize}
...
\begin{verbatim}
# Process PDF with enhanced strategy
python cli.py book.pdf --max-pages 10 --model gpt-4o

# Use consensus strategy for maximum accuracy
python cli.py book.pdf --strategy consensus --model claude-3-5-sonnet

# Process specific page range
python cli.py book.pdf --start-page 5 --end-page 15 --save-crops
\end{verbatim}
...
# Simple usage
results = process_pdf_to_page_fens("book.pdf", model="gpt-4o")
...
\begin{itemize}
    ...
    \item \textbf{VLM Evaluation:} Python harness using OpenAI GPT-4o API
